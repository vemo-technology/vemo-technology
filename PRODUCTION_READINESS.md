...
- Le dépôt Supabase est lié à la référence locale enregistrée, mais la base distante n'est pas vérifiable avec l'état réseau/identifiants actuels.
...
- Supabase : appliquer et vérifier la migration `20260718224000_production_schema.sql`. La CLI demande le mot de passe PostgreSQL, absent localement, et `dnkxugrfqrsafiwssfdm.supabase.co` renvoie `ENOTFOUND`. Confirmer la bonne référence projet avant toute migration.
...
- Vercel : aucune liaison `.vercel` ni session CLI exploitable n'a pu être prouvée sur cette machine. Configurer Preview et Production avec toutes les variables de `.env.example` et Node.js 22.
